...
function Dashboard({ user, logout }) {
  ...
  // Show redirect notice from protected pages
  useEffect(() => {
  ...
  async function shareFileAccess() {
    ...
    try {
      ...
      if (response.ok) {
        setShareMessage(response.ok ? (data?.message || "File shared successfully") : getFriendlyShareErrorMessage(data?.message || data?.error, response.status));
        setDashboardMessage("Sharing permission updated successfully.");
        setShareFileId("");
        setTargetUserEmail("");
        loadMyFiles();
      } else {
        setShareMessage(response.ok ? (data?.message || "Share failed") : getFriendlyShareErrorMessage(data?.message || data?.error, response.status));
      }
    } catch (error) {
